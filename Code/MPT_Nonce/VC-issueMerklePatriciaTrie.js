...
/**
 *
 * Install package: npm install merkle-patricia-tree
 *
 */
...
async function createVCPayload(user,nClaims) {
    ...
    const newPatriciaMerkleTree = await createPatriciaMerkleTree(credAttrName,credAttrValue); // (attributi da criptare) --> restituisce un oggetto contenente (path nodi per proof, root del merkle patricia trie, lista nonce}
...
const test = async (accounts) => {
    ...
    const timeElapsed = []
    console.log("-------------------------------------------------------------------------")
    console.log("HLeaves: " + config.merklePatriciaTree.HLeaves)
    console.log("HTree: " + config.merklePatriciaTree.HTree)
    console.log("Numero iterazioni: " + config.merklePatriciaTree.iterations)
    console.log("-------------------------------------------------------------------------")
    for (let i = 1; i <11; i++) {
        let res = 0.0;
        let jwtP;
        ...
        for (let j = 0; j < config.merklePatriciaTree.iterations; j++) {
        ...
        vcCreationTimes.push([res/config.merklePatriciaTree.iterations, jwtSize/config.merklePatriciaTree.iterations, getStandardDeviation(timeElapsed)]);
        console.log("-------------------------------------------------------------------------")
        // console.log(timeElapsed)
        console.log("Numero attributi credenziale: " + nCl)
        console.log("Numero iterazioni: " + config.merklePatriciaTree.iterations)
        console.log("La sua media è: " + res/config.merklePatriciaTree.iterations)
    ...
    const file = fs.createWriteStream('3-MPT-VC-issue-Lsha3-Tkeccak.txt');
