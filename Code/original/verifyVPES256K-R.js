...
console.log("Verify credential");
	...
	for (let i = 1; i < 11; i++) {
		...
		let jwtP=await createVerifiablePresentationJwt(VPPayload,PaoloMori,options);
		for (let j = 0; j <500; j++) {
			let start = performance.now();
				...
				//console.log(verifiedPresentation.verifiablePresentation.verifiableCredential);
	...
	const fs = require('fs')
	const file = fs.createWriteStream('verifyVPES256K-R.txt');
