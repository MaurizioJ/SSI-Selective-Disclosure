...
const test = async (accounts) => {
	...
	for (let i = 1; i < 11; i++) {
		...
		const atomicVCPayloads = await createVCPayload(PaoloMori,Math.pow(2, i));
		let jwtSet =[];
		...
		const VPPayload=createVPPayload(jwtSet,nCl);
		...
		for (let j = 0; j <config.atomic.iterations; j++) {
  		...
  		vcCreationTimes.push([res/config.atomic.iterations]);
  	}
	
	console.log(vcCreationTimes);

	const file = fs.createWriteStream('verifyAtomicVCP-ES256K.txt');
