...
// We import the library required to create a basic Merkle Patricia Tree
...
function createVPPayload(vc,nClaims) {
    ...
    if (nClaims==disclosure.clearKeyList.length) { // caso in cui devo rivelare tutti gli attributi
        ...
        for (let i = 0; i < nClaims; i++) {
            ...
            let listPathNodesComp = zlib.brotliCompressSync(buf) //2. applicazione algoritmo di compressione sulla proof
            // console.log("proof creata dall' issuer")
...
const test = async (accounts) => {
    ...
    const timeElapsed = []
    console.log("-------------------------------------------------------------------------")
    console.log("Verify Compression Proof")
    ...
    for (let i = 10; i <11; i++) {
        ...
        vcCreationTimes.push([res/config.merklePatriciaTree.iterations, getStandardDeviation(timeElapsed)]);
        console.log("-------------------------------------------------------------------------")
    ...
    const file = fs.createWriteStream('2-Compression-MPT-VP-verify-Lsha3-T-none.txt');
