    ...
    VCPayload['vp']['proof']=[]
     if (nClaims===disclosure.clearValueList.length) { // caso in cui devo rivelare tutti gli attributi 
        ...
        for(var elem of disclosure.clearValueList){
            ...
            let proofJs = MerkleTree.marshalProof(disclosure.proof[name]) // Returns proof array as JSON string
            var listPathNodes = zlib.brotliCompressSync(proofJs) // applicazione algoritmo di compressione sulla proof
            listPathNodes= listPathNodes.toString('base64') // view a string per compatibilità con jwt
        ...
        for (let i = 0; i < nClaims; i++) {
            ...
            let proofJs = MerkleTree.marshalProof(disclosure.proof[name]) // Returns proof array as JSON string
            var listPathNodes = zlib.brotliCompressSync(proofJs) // compressione della proof
            listPathNodes= listPathNodes.toString('base64') // view a string per compatibilità con jwt
            var nonceLeaves= disclosure.nonce[name]
...
const test = async (accounts) => {
    ...
    let j=1
    console.log("-------------------------------------------------------------------------")
    console.log("HLeaves: " + config.merkleTree.HLeaves)
    console.log("HTree: " + config.merkleTree.HTree)
    console.log("Numero iterazioni: " + config.merkleTree.iterations)
    console.log("Percentuale attributi svelati della VC: " + config.merkleTree.soglia)
    console.log("-------------------------------------------------------------------------")
    for (let i = 1; i <11; i++) {
        let res=0.0;
        let jwtP;
       ...
       /* console.log("VP")
        ...
        for (let j = 0; j <config.merkleTree.iterations; j++) {
            ...
            timeElapsed.push(createVCtime) // array contenente i tempi
        ...
        vcCreationTimes.push([res/config.merkleTree.iterations, getStandardDeviation(timeElapsed)]);
        console.log("-------------------------------------------------------------------------")
        // console.log(timeElapsed)
        console.log("Numero attributi credenziale: " + nCl)
        console.log("Numero iterazioni: " + config.merkleTree.iterations)
        console.log("La sua media è: " + res/config.merkleTree.iterations)
        console.log("La sua deviazione standard è: " + getStandardDeviation(timeElapsed))
        console.log("-------------------------------------------------------------------------")

    }

    console.log(vcCreationTimes);
    const file = fs.createWriteStream('2-Compression-MT-VP-verify-Lsha256-Tkeccak.txt');
    ...
    // var file = require('fs');
    //
    // file.appendFile('2-MT-VP-verify-Lsha256-Tkeccak.txt', Math.pow(2,j) + ' ' + vcCreationTimes + '\n', function (err) {
    //     if (err) throw err;
    //     console.log('Saved!');
    // });
